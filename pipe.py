...
def handle_redirection(command, _input, flag):
    ...
    for i in range(len(command)):
        if command[i] in indicators:
            ...
            if command[i] == ">" and command[i-1] != ">" and\
                    command[i+1] != ">":
    ...
    if flag is True:
        sub.run(commands_to_do, stdout=sys.stdout, input=_input,
                stderr=sys.stderr)
        return None
    child = sub.run(commands_to_do, stdout=sub.PIPE, input=_input,
                    stderr=sys.stderr)
    return child.stdout
...
def handle_pipe(args):
    ...
    for i in range(len(commands)):
        ...
        temp_pipe = handle_redirection(command, temp_pipe,
                                       i == len(commands)-1)
        '''  receives the original sys.stdout, sys.stdin,
         sys.stderr stored before  '''
